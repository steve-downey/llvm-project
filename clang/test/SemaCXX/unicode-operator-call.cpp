...
//
//   * ADL (U6), which is normative rather than a quality-of-implementation
//     matter. An operator use must find every overload the plain call
...
//===--------------------------------------------------------------------===//
// 8. Composability with other user-introduced infix syntax (U7)
//===--------------------------------------------------------------------===//

// The U7 ground rule is that this feature is independent of any other
// user-introduced infix syntax sharing prec::UserInfix: every assertion in
// this file must hold identically whether or not such a feature is also
// enabled. There is no second such feature in tree, so the claim is vacuous
// here rather than untested -- it is checked on the prototype branch that
// carries one.
