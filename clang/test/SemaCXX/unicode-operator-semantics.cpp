...
//
// Item 9 ("both flags on, both off, and each alone") is the RUN lines: the
// feature is exercised with -funicode-operators alone, with -fbacktick added,
// under both constant evaluators, and with the flag off entirely -- twice,
// once with -fbacktick and once without, since U7 makes the two independent.
...
// RUN: %clang_cc1 -std=c++23 -funicode-operators -fbacktick -DBACKTICK -fexperimental-new-constant-interpreter -fsyntax-only -verify %s
// RUN: %clang_cc1 -std=c++23 -DOFF -fsyntax-only -verify=off %s
// RUN: %clang_cc1 -std=c++23 -fbacktick -DOFF -fsyntax-only -verify=off %s
...
//
// D15's other half -- "since C++17 the callee is sequenced before both
// operands, so the slot is evaluated first even though it is written between
// them" -- has no analogue here. Backtick's slot is an *expression* and can
// have side effects; a Unicode operator's callee is a name, so there is
// nothing for [expr.call]p8's first sentence to sequence.
...
//===----------------------------------------------------------------------===//
//
// The RUN lines are the flag matrix; this block is the part that only exists
// when both features are on. The two share one precedence level and no
// semantics: each is independently the call it desugars to.

constexpr int fn(int a, int b) { return 10 * a + b; }

static_assert((1 `fn` 2) == 12);
static_assert((1 ⊟ 2) == 4);
static_assert((⊯1 `fn` 2) == 42);       // prefix user operator as a backtick operand
static_assert((1 ⊟ 2 `fn` 3) == 43);    // one level, left-associative

// The backtick slot is an assignment-expression, and an operator-function-id
// is an id-expression, so a user operator's overload set can BE the slot.
static_assert((1 `operator⊟` 2) == 4);
static_assert((1 `operator⊟` 2) == (1 ⊟ 2));

// noexcept, through both syntaxes, from the same callee.
constexpr int nofail(int, int) noexcept { return 0; }
static_assert(noexcept(1 `nofail` 2));
static_assert(noexcept(1 ⊣ 2));
#endif

#else
//===----------------------------------------------------------------------===//
// 9. Flag off (with and without -fbacktick)
//===----------------------------------------------------------------------===//
//
// Without -funicode-operators the code points are not operator tokens and not
// identifier characters either, in a translation unit with -fbacktick just as
// in one without: the two flags are independent (U7). One representative
// declaration and one representative use, infix and prefix.
