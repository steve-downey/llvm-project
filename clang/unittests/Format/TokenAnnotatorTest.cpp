...
TEST_F(TokenAnnotatorTest, UnicodeOperatorTokenTypes) {
  ...
  // Mixed with the backtick infix operator (the two features are independent
  // but compose).  A keyword-escape closes an identifier, so what follows is
  // infix; a backtick infix close is followed by the right operand, so what
  // follows there is prefix.
  Tokens = annotate("a `f` b ⊞ c;");
  ASSERT_EQ(Tokens.size(), 9u) << Tokens;
  EXPECT_TOKEN(Tokens[5], tok::user_operator, TT_BinaryOperator);
  Tokens = annotate("`new` ⊞ b;");
  ASSERT_EQ(Tokens.size(), 7u) << Tokens;
  EXPECT_TOKEN(Tokens[3], tok::user_operator, TT_BinaryOperator);
  Tokens = annotate("a `f` ⊖b;");
  ASSERT_EQ(Tokens.size(), 8u) << Tokens;
