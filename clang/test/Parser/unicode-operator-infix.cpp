...
// --- composability with -fbacktick (U7): one level, shared ---------------
// Both features on: the mixed chain groups left across the two spellings,
// which is the claim U§12 rests on.  Everything above this point is also
// the *other* direction -- it runs with -fbacktick off.
#ifdef BACKTICK
constexpr int f(int a, int b) { return 5 * a + b; }

// f(operator⊞(1, 2), 3); right-associative would be 2*1 + f(2,3) == 15.
static_assert(1 ⊞ 2 `f` 3 == 23);
// operator⊞(f(1, 2), 3); right-associative would be f(1, 2*2+3) == 12.
static_assert(1 `f` 2 ⊞ 3 == 17);
static_assert(1 ⊞ 2 `f` 3 ⊞ 4 == 50);   // ((1 ⊞ 2) `f` 3) ⊞ 4
static_assert(2 * 3 `f` 4 ⊞ 5 == 86);    // 2 * ((3 `f` 4) ⊞ 5)

int ast_mixed(int a, int b, int c) { return a ⊞ b `f` c; }
// BT-LABEL:  FunctionDecl {{.*}} ast_mixed
// BT:        BacktickInfixExpr
// BT:        DeclRefExpr {{.*}} 'f'
// BT:        CallExpr
// BT:        DeclRefExpr {{.*}} 'operator⊞'
#endif
